...
class PackageManager
{
    ...
    function __construct()
    {
        ...
        foreach (glob($this->appDirectory . 'packages/*/config.json') as $configFile) {
            if ($package = json_decode(file_get_contents($configFile))) {
                ...
                $this->packages[$package->id]->code_required = in_array($package->type, ['game', 'add-on']) && $package->id != 'baccarat';
    ...
    /**
     * Load version info for each enabled package from the package config file.
     * This can only be called after the package service provider is registered.
     *
     * @return PackageManager
     */
    public function loadVersions(): PackageManager
    {
